...
for i in range(10,21,1):
	for j in range(10,21,1):
		number=str(i)+str(j)   	   	
		filename = pwd +"/ganglion_MI/"+ number + ".jpg"        

		read_ganglion = np.loadtxt(pwd+"/results/SNL_ganglion"+number)
		...
		for t in taus:
			...
			ganglion = read_ganglion[-int(sim_time/0.001):]
		
			ganglion = ganglion[Forward-time_shift:len(ganglion)-Backward-time_shift]
			shift_ganglion = ganglion.reshape(-1, 50).mean(axis=1)
		
			shift_tss = binning(nbin,shift_ganglion)
			xedge = range(nbin+1)
			yedge = range(nbin+1)
			pxy, xedges, yedges = np.histogram2d(tss,shift_tss,bins=(xedge,yedge))
			mik.append(MI(pxy))

			stt_shuffle = np.random.permutation(shift_tss)#have serious problem
...
np.save('peak_ganglion.npy',peak_time)
print (peak_time)
