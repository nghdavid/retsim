...
for i in range(10,21,1):
	for j in range(10,21,1):
		number=str(i)+str(j)   	   	
		filename = pwd +"/Tp_MI/"+ number + ".jpg"        

		read_Tp = np.loadtxt(pwd+"/results/tmp_photoreceptors"+number)
		...
		for t in taus:
			...
			Tp = read_Tp[-int(sim_time/0.001):]
		
			Tp = Tp[Forward-time_shift:len(Tp)-Backward-time_shift]
			shift_Tp = Tp.reshape(-1, 50).mean(axis=1)
		
			shift_tss = binning(nbin,shift_Tp)
			xedge = range(nbin+1)
			yedge = range(nbin+1)
			pxy, xedges, yedges = np.histogram2d(tss,shift_tss,bins=(xedge,yedge))
			mik.append(MI(pxy))

			stt_shuffle = np.random.permutation(shift_Tp)
